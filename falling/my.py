...
class objects(pygame.sprite.Sprite):
    ...
    def draw(self):
        if self.hit:
            screen.blit(self.happy,self.happy_rect)
            self.count+=1
        elif self.dead:
            screen.blit(self.skull,self.skull_rect)
            self.count+=1
        # Blit the falling object onto the specified surface
        else:
            screen.blit(self.rope, self.rope_rect)
            screen.blit(self.fan, self.fan_rect)
            # pygame.draw.rect(screen, self.border_color, self.fan_rect, self.border_width)
            # pygame.draw.rect(screen, self.border_color, self.rope_rect, self.border_width)
            # pygame.draw.rect(screen, self.border_color, self.hangman_rect, self.border_width)
            screen.blit(self.hangman, self.hangman_rect)          
...
background = pygame.image.load('falling/images/cloud.jpg')  
background = pygame.transform.scale(background, (WIDTH, HEIGHT))                                #game background
...
def hide_cross_lives(x, y):
    screen.blit(pygame.image.load("falling/images/red_lives.png"), (x, y))
...
def line_rect_collision(line_start, line_end, rect):
    ...
    if(len(rect.clipline(line_start,line_end))): 
        return True
    return False
...
def run2(screen):
    ...
    drawing = False
    background2 = pygame.image.load('falling/images/sky.jpg').convert()
    ...
    cut_sound = pygame.mixer.Sound('game_sound/cut.mp3')
    cut_sound.set_volume(0.3)

    death_sound = pygame.mixer.Sound('game_sound/death.mp3')
    ...
    while True : 
            ...
            if event.type == pygame.MOUSEMOTION:
                if drawing:
                    ...
                    if last_pos is not None:
                        pygame.draw.line(screen, (255,0,0), (last_pos[0], last_pos[1]), (mouse_position[0], mouse_position[1]), w)
        ...
        for line in lines:
            start_pos, end_pos, timestamp= line
            pygame.draw.line(screen, (0,0,200), (start_pos[0],start_pos[1]), (end_pos[0], end_pos[1]), 2*w)
            pygame.draw.line(screen, (255,255,255), (start_pos[0],start_pos[1]), (end_pos[0], end_pos[1]), w)
        ...
        if game_over :
            ...
            # print("yes")
            draw_text(screen, "SUICIDE NINJA!", 90, WIDTH / 2, HEIGHT / 4)
            draw_text(screen, "Press a key to begin!", 64, WIDTH / 2, HEIGHT * 3 / 4)
            player_lives = 3
            # draw_lives(screen, 690, 5, player_lives, 'Fruit/images/red_lives.png')
            score = 0
            ...
            # pygame.draw.rect(screen, border_color, rope_rect, border_width)
            # pygame.draw.rect(screen, border_color, fan_rect, border_width)
            # pygame.draw.rect(screen, border_color, hangman_rect, border_width)
            # pygame.draw.rect(screen, border_color, slash_rect, border_width)
            draw_lives(screen, 890, 5, player_lives, 'falling/images/red_lives.png')
